...
@cocotb.test()
async def test_law(dut):
	"""Test AR channel functionality"""
	...
	bus = axi.AxiLiteBus.from_prefix(dut, "M_AXI")

	# AxiSlave生成（Master側とつなぐ）
	axi_slave = axi.AxiLiteRam(bus, dut.M_ACLK, dut.M_ARESETN, size=4096, reset_active_level=False)
	...
	# Reset DUT
	dut.i_start_address.value = 0x10
	dut.i_write_size.value = 128 # 16B
	dut.M_ARESETN.value = 0
